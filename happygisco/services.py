...
class GISCOService(OSMService):
    ...
    def url_nuts(self, source=None, **kwargs):
        ...
        available from |GISCO| Rest API and/or bulk download website:
            ...
            >>> serv.url_nuts('BULK', year = 2016, scale = 60, ifmt = 'shp')
        ...
        from the preformated |Nuts2json| data collection:
            ...
            >>> serv.url_nuts('nuts2json', proj = 'EPSG3035', year = 2010, ifmt = 'topojson')
    ...
    def country_response(self, **kwargs):
        """Download, and cache when requested, country vector files from |GISCO| Rest API.
        ...
        if source == 'INFO':
            ...
            # ['SOURCE', 'YEAR', 'PROJECTION', 'SCALE', 'IFORMAT']
    ...
    def nuts_response(self, **kwargs):
        ...
        Examples
        --------
        The method can be used to retrieve well-defined responses from |GISCO| Rest API:
        ...
        if source == 'BULK':
            ...
            # ['SOURCE', 'YEAR', 'PROJECTION', 'SCALE', 'VECTOR', 'LEVEL', 'IFORMAT']
            pass
        else:
            # ['SOURCE', 'UNIT', 'YEAR', 'PROJECTION', 'SCALE', 'VECTOR', 'IFORMAT']
            [dimensions.remove(attr) for attr in ('LEVEL',)] 
        dimensions = collections.OrderedDict(zip(dimensions,[None]*len(dimensions)))        
        for attr in dimensions.keys():
            val = kwargs.pop(getattr(_Decorator,'KW_' + attr), None)
        ...
        if source == 'NUTS2JSON':
            if 'IFORMAT' in dimensions:
                dimensions.update({'IFORMAT': ['topojson' if f=='json' else f        \
                                                          for f in dimensions.get('IFORMAT')]})
        ...
        * It is recommended, for efficiency reasons, to set the option :data:`_caching_=True`
        ...
        if info == 'NAMES':
            ...
            kwargs.update({_Decorator.KW_SCALE: scale,
                           _Decorator.KW_IFORMAT: 'shp', # instead of settings.DEF_GISCO_FORMAT since 'shp' is lighter
    ...
    def url2dimension(self, url, **kwargs):
        ...
        automatically generated URLs (reversing the generation process):
            ...
            >>> serv.url2dimension(url)
                OrderedDict([('source', 'AT'),
                             ...
                             ('vector', 'RG'),
                             ('ifmt', 'geojson')])
            >>> url = serv.url_nuts(source='NUTS', level=3, ifmt='shp')
            >>> print(url)
            ...
            >>> serv.url2dimension(url)
                OrderedDict([('source', 'NUTS'),
                             ...
                             ('level', 3),
                             ('ifmt', 'shp')])
            >>> url = serv.url_nuts(source='NUTS', level='ALL', vector='label', ifmt='geojson')
            >>> print(url)
            ...
            >>> serv.url2dimension(url)
                OrderedDict([('source', 'NUTS'),
                             ...
                             ('level', 'ALL'),
                             ('ifmt', 'geojson')])
            >>> url = serv.url_nuts(source='NUTS2JSON', ifmt='geojson')
            >>> print(url)
        ...
        Country URL are also recognised:
            
            >>> url = 'https://ec.europa.eu/eurostat/cache/GISCO/distribution/v2/countries/geojson/CNTR_RG_20M_2010_3857.geojson'
            >>> serv.url2dimension(url)
                OrderedDict([('source', 'CNTR'),
                             ('year', 2010),
                             ('proj', 3857),
                             ('scale', '20M'),
                             ('vector', 'RG'),
                             ('ifmt', 'geojson')])
            >>> url='https://ec.europa.eu/eurostat/cache/GISCO/distribution/v2/countries/geojson/CNTR_BN_20M_2010_3035_INLAND.geojson'
            >>> serv.url2dimension(url)
                OrderedDict([('source', 'CNTR'),
                             ('year', 2010),
                             ('proj', 3035),
        ...
        Note the usage of the keyword argument :data:`_force_list_` to format the
        output:
                
            >>> url = serv.url_nuts(source='NUTS2JSON', ifmt='geojson')
            >>> serv.url2dimension(url, _force_list_=True)
                OrderedDict([('year', [2013]),
                             ('proj', [3857]),
                             ('scale', ['60M']),
                             ('vector', ['RG']),
        ...
        See also
        --------
        :meth:`~GISCOService.url_nuts`, :meth:`~GISCOService.geom2dimension`.
        """
        ...
        if isgisco > 0:
            ...
            if turl.find(settings.GISCO_PATTERNS['bulk']['domain']) >= 0:
                # source = 'BULK'
                if turl.find(settings.GISCO_PATTERNS['country']['bulk']) > 0:
                    sub = sub[0].replace(settings.GISCO_PATTERNS['country']['bulk'],'').split('-')
                elif turl.find(settings.GISCO_PATTERNS['nuts']['bulk']) > 0:
                    sub = sub[0].replace(settings.GISCO_PATTERNS['nuts']['bulk'],'').split('-')
                year, scale = sub
            elif (turl.find(settings.GISCO_PATTERNS['country']['fmt']) >= 0         \
                    and turl.find(settings.GISCO_PATTERNS['country']['info']) > 0)  \
                  or (turl.find(settings.GISCO_PATTERNS['nuts']['fmt']) >= 0        \
                    and turl.find(settings.GISCO_PATTERNS['nuts']['info']) > 0):
                # source = 'INFO'
                year = sub[0].split('-')[1]
            elif turl.find(settings.GISCO_PATTERNS['country']['base']) >= 0:
                source = 'CNTR'
                sub = sub[0].replace(settings.GISCO_PATTERNS['country']['base'],'').split('_')
                vec = sub[0]
                if vec == 'LB':         year, proj = sub[1:3]
                else:                   scale, year, proj = sub[1:4]
                if len(sub)==5:         level = sub[-1] # 'INLAND' or 'COASTL'
                else:                   level = None 
            ...
            vec = 'RG' if vec in ('nutsrg','cntrg') else ('LB' if vec in ('nutsbn','cntbn') else vec)
        # ['UNIT', 'YEAR', 'PROJECTION', 'SCALE', 'VECTOR', 'LEVEL', 'IFORMAT']
        kwargs = {}
        if not(source is None and unit is None):
            kwargs.update({'SOURCE': source or unit})
        if year is not None:
        ...
        if level is not None:
            kwargs.update({'LEVEL': int(level) if happyType.isnumeric(level) else level})
        if fmt is not None:
            kwargs.update({'IFORMAT': fmt})  
        ...
        [dimensions.update({getattr(_Decorator, 'KW_' + k): kwargs.get(k)})  \
             or dimensions.pop(k) if k in kwargs else dimensions.pop(k) for k in list(dimensions.keys())]
        if __force_list is True:
            [dimensions.update({k: v if happyType.issequence(v) else [v]}) for k,v in list(dimensions.items())]
        return dimensions
    
    #/************************************************************************/
    def geom2dimension(self, geom, **kwargs):
        """Check whether a given URL represents a |NUTS| dataset disseminated though
        |GISCO| Rest API and return information about it.
            
            >>> dimensions = serv.geom2dimension(geom)
        ...
        output:
            ...
            >>> geom = serv.coord2nuts([41.8933203,12.4829321], level=2)  
            >>> serv.geom2dimension(geom, _force_list_=True)
                OrderedDict([('source', ['ITI4']), 
                             ('year', ['2013']), 
                             ('level', ['2'])])
        ...
        See also
        --------
        :meth:`~GISCOService.url2dimension`.
        """
        __force_list = kwargs.pop(_Decorator.KW_FORCE_LIST, False)
        try:
        ...
        dimensions = collections.OrderedDict(zip([getattr(_Decorator, 'KW_' + k) for k in settings.GISCO_DATA_DIMENSIONS],
                                                 [None]*len(settings.GISCO_DATA_DIMENSIONS)))
        dimensions.pop(_Decorator.KW_IFORMAT)        
        if _Decorator.parse_nuts.KW_ATTRIBUTES in geom.keys():
            ...
            if __force_list is True:
                [dimensions[i].update({k: v if happyType.issequence(v) else [v]})  \
                     for k,v in list(dimensions.items()) for i in range(len(features))]
